...
namespace TestFormLoaiPhong
{
    ...
    {
        private string connString = "Data Source=DESKTOP-0A82EOD\\MSI;Initial Catalog=QLKS99;Integrated Security=True;Encrypt=True;TrustServerCertificate=True;";
        ...
        private void LoadData()
        {
            try
            {
                using (SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM vw_LoaiPhong", connString))
                {
                    ...
                    if (dgvLoaiPhong.Columns["TrangThaiText"] != null)
        ...
        private void FillInputsFromRow(DataGridViewRow row)
        {
            ...
            string trangThai = row.Cells["TrangThaiText"].Value?.ToString() ?? "Đang dùng";
